...
- **Fourth external audit adopted (2026-09-02)**: Grok 4.6 and Kimi seats on `8bcc5d0`; nine findings adopted with a 17-case regression module (A20–A28), two declined with reasons. The headline: `--safe-mode` never removed built-in tools — `--restricted` does, and is now load-bearing. Details in `VALIDATION.md`.
