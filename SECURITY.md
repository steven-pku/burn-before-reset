...
- The planner never treats this tool's own output as source material. A file inside `run.output_root`, a session directory whose name encodes a working directory inside it, and a transcript that declares such a working directory are all dropped before scoring, and each drop is named in `RUN_PLAN.md`. This holds for every run sharing the output root, not only the current one, and does not depend on the operator writing an exclusion. Until 2026-09-08 only movement detection was guarded this way, and a re-planning round queued work against the run's own worker transcripts.
- `exclude_fragments` entries are matched as a case-insensitive substring of the path relative to the source root. Until 2026-09-08 they were intersected against whole path components, so an entry could only ever match a complete directory or file name — on a session source, where a whole project path is flattened into one segment, no entry could express "exclude this project" at all. A `/` at either end of an entry anchors it to a whole path segment, so a directory-name entry such as `/.git/` does not start catching `.github/`. `bbr validate-config` now counts what each entry matches and warns about any entry that matches nothing; an inert exclusion is indistinguishable from a working one until it is counted.
