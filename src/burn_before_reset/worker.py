...
def _worker_command(config: AppConfig, prompt: str, staging: Path, source_roots: list[Path]) -> list[str]:
    ...
    if config.execution.provider == "claude":
        command = [
            ...
            # the second lock on the same door. The flag is documented as of 2026-09-06; installed CLI
            # versions vary, so preflight probes `claude --help` for it and every other
