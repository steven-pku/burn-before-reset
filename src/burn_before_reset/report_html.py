...
CHROME: dict[str, dict[str, Any]] = {
    "en": {
        ...
        "sample_notice": "SAMPLE DATA · No model calls · Illustrative artifacts",
        ...
        "facts": {"completed": "Tasks completed", "artifacts": "Artifacts", "spent": "Usage cost estimate",
        ...
        "archetype": {
            "decision": ("decision briefs", "review the options, evidence and gaps before deciding"),
            "verify": ("claim reviews", "read each verdict: confirmed, refuted, or uncheckable"),
        ...
        "foot": "Dollar amounts are CLI-reported usage estimates, not bills, savings, remaining quota or artifact value. Generated by Burn Before Reset — machine-readable twin: MORNING_REPORT.md · receipts: RUN_STATE.json, events.jsonl",
    ...
    "zh": {
        ...
        "sample_notice": "示例数据 · 未调用模型 · 产物仅供演示",
        ...
        "facts": {"completed": "完成任务", "artifacts": "产出成果", "spent": "用量估算金额",
                  "tokens": "输出 tokens", "hours": "用时", "stop": "停机方式"},
        "sections": {
            "first": ("先看这里", "按工作类型分组，决策排最前。"),
            "work": ("全部成果", "每一份成果和它的来路。加入清单后，复制给你的 agent 继续处理。"),
            "watch": ("值守纪事", "这一趟的时间是怎么花的，逐分钟。"),
            "ledger": ("安全与账目", "它承诺不做的事，和每一张回执。"),
        },
        "attention_empty": "没有等拍板的事，可以慢慢读。",
        "archetype": {
            "decision": ("决策分析", "先审阅选项、证据和缺口，再做决定"),
            "verify": ("主张核验报告", "逐份查看结论：证实、证伪或暂时无法核验"),
        ...
        "foot": "金额是 CLI 回报的用量估算，不代表账单、节省金额、剩余额度或成果价值。由 Burn Before Reset 生成 — 机器可读版：MORNING_REPORT.md · 回执：RUN_STATE.json、events.jsonl",
...
def write_html_report(run_dir: Path, state: dict[str, Any], *, language: str = "auto") -> Path:
    ...
    page = (
        ...
        + _band(state)
        + '<div class="wrap">\n'
        + (f'<p class="kicker">{_esc(chrome["sample_notice"])}</p>' if state.get("sample_data") else "")
